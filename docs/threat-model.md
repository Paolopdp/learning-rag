...
| TM-16 | Query abuse (high request rate) causing resource exhaustion or noisy workloads | Per-workspace throttling with `429` + `Retry-After` and audited denials; Redis/Valkey-backed counters by default with resilient in-memory fallback | `backend/tests/test_rate_limit.py`, `backend/app/rate_limit.py` | Partially mitigated | Tune limits per role/use-case and add SLO-driven thresholds/alerting. |
| TM-17 | Upload ingestion resource exhaustion (large files/PDF parsing/embedding spikes) | In-app upload guardrails (`RAG_INGEST_MAX_FILES`, `RAG_INGEST_MAX_FILE_BYTES`, `RAG_INGEST_MAX_PDF_PAGES`, `RAG_INGEST_MAX_PDF_TEXT_CHARS`) plus early `400` validation responses | `backend/tests/test_ingest_upload.py::test_ingest_upload_rejects_file_too_large`, `backend/tests/test_ingest_upload.py::test_ingest_upload_rejects_too_many_files`, `backend/tests/test_ingestion.py::test_parse_uploaded_pdf_file_uses_pdf_extractor` | Partially mitigated | Add async/offline ingestion queue for heavier workloads and per-workspace quotas. |
